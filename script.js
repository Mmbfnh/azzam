...
function getLevelItems(){ return [...ITEMS].sort(()=>Math.random()-0.5).slice(0, levelConfig[currentLevel].count); }
...
function renderMatch(){ matchImages.innerHTML=''; matchWords.innerHTML=''; assignments={}; const subset=getLevelItems(); matchTotal=subset.length; subset.forEach((item,idx)=>{ const slot=document.createElement('div'); slot.className='slot'; slot.dataset.slotId='slot_'+idx; slot.dataset.itemId=item.id; const img=document.createElement('img'); img.className='figure'; img.src=getImgSrc(item); img.alt=item[langSelect.value]; const label=document.createElement('div'); label.className='drop-label'; label.textContent=(langSelect.value==='ar')?'اسحب الكلمة هنا':'Drag the word here'; slot.addEventListener('dragover', e=>{ e.preventDefault(); slot.classList.add('dragover'); }); slot.addEventListener('dragleave', ()=> slot.classList.remove('dragover')); slot.addEventListener('drop', e=>{ e.preventDefault(); slot.classList.remove('dragover'); const droppedId=e.dataTransfer.getData('text/plain'); if(!droppedId) return; const existing=slot.querySelector('.chip'); if (existing) matchWords.appendChild(existing); const chip=document.getElementById('chip_'+droppedId); if (chip){ slot.appendChild(chip); chip.classList.remove('dragging'); assignments[slot.dataset.slotId]=droppedId; updateScore(matchTotal); } }); slot.appendChild(img); slot.appendChild(label); matchImages.appendChild(slot); }); const words=[...subset].sort(()=>Math.random()-0.5); words.forEach(item=>{ const chip=document.createElement('div'); chip.className='chip'; chip.id='chip_'+item.id; chip.textContent=item[langSelect.value]; chip.draggable=true; chip.addEventListener('dragstart', e=>{ chip.classList.add('dragging'); e.dataTransfer.setData('text/plain', item.id); }); chip.addEventListener('dragend', ()=> chip.classList.remove('dragging')); matchWords.appendChild(chip); }); updateScore(matchTotal); }
...
function initLetters(){ pickRandomLettersItem(); }
function pickRandomLettersItem(){ currentLettersItem=ITEMS[Math.floor(Math.random()*ITEMS.length)]; buildLettersRound(); }
...
// Init default
showSection(cardsMode); render(); updateTimerDisplay();
